...
BotData.GlobalData = {
	...
	Levels: {
		ExpPerChat: 3,
		ExpNeeded: [
			0, 8, 14, 25, 45, 81, 146, 263, 473, 851, 1532, 2758,
			4964, 8935, 16083, 28949, 52108, 93794, 168829,
			303892, 547006, 984611, 1772300, 3190140, 5742252,
			10336054, 18604897, 33488815, 60279867, 108503761,
			195306770, 351552186, 632793935, 1139029083,
			2050252349, 3690454228, 6642817610, 11957071698,
			21522729056, 38740912301, 69733642142, 125520555856,
			225937000541, 406686600974, 732035881753,
			1317664587155, 2371796256879, 4269233262382,
			7684619872288, 13832315770118, 24898168386212,
			44816703095182, 80670065571328, 145206118028390,
			261371012451102, 470467822411984, 846842080341571,
			1524315744614830, 2743768340306690, 4938783012552040,
			8889809422593670, 16001656960668600, 28802982529203500
		]
	},
	// Save path and token
		...
		function: function(message, _) {
			message.channel.send({
				embed: {
					...
					description: `Say \`${BotData.ServerData.Prefix || BotData.GlobalData.DefaultPrefix}help\` for a list of commands.`,
					...
					fields: [
						{
							name: "Prefix",
							value: `My prefix for this server is \`${BotData.ServerData.Prefix || BotData.GlobalData.DefaultPrefix}\`.`,
							inline: true
						},
						{
							name: "Developer",
		...
		function: function(message, _) {
			...
			message.author.send({
				embed: {
					...
					description: `My prefix for ${message.guild.name} is \`${BotData.ServerData.Prefix || BotData.GlobalData.DefaultPrefix}\``,
					fields: commands
				}
			}).catch(Log);
		}
